window.onload = async function () {
  ...
  if (!comentList) {
    ...
    comentList.forEach((coment) => {
      ...
      // btns.className = 'flex items-center justify-between size-auto'

      alterado = coment.com_alterado;
      if(alterado){
        const tag = document.createElement('span');
        tag.className = 'text-sm bg-purple-100 text-purple-700 px-3 py-1 rounded-sm ';
      ...
      btnDeletar.className = "ml-4 mt-1 cursor-pointer";
      btnDeletar.onclick = () => {
        Swal.fire({
      ...
      btnEditar.className = "ml-4 mt-1 cursor-pointer";
      ...
      `;
      btnDenunciar.className = "ml-4 mt-1 cursor-pointer";
      btnDenunciar.onclick = () => {
        Swal.fire({
            title: "Você deseja denunciar o comentario?",
            text: "Você não poderá reverter isso!",
            icon: "warning",
            showCancelButton: true,
            confirmButtonColor: "#6F23D9",
            ...
            if (res.isConfirmed) {
              idcom = coment.com_id
              window.location.href = `/trevo-project/public/denunciar?tipo=com&&id=${idcom}`;
            }
          });
      }
